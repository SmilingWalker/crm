...
<%@ page import="com.frank.crm.settings.domain.DictValue" %>
<%@ page import="java.io.IOException" %>
<%@ page import="com.fasterxml.jackson.databind.ObjectMapper" %>
<%@ page import="com.frank.crm.workerbench.domain.Tran" %>
...
<script type="text/javascript">
	...
	function refreshStageIcon(CurrentStage){
		<%
		List<String> stageList = new ArrayList<>();
		List<DictValue> dictValues = (List<DictValue>) application.getAttribute("stage");
		ObjectMapper om = new ObjectMapper();
		for (DictValue d:dictValues){
			String Str = "";
			try {
				Str = om.writeValueAsString(d);
			} catch (IOException e) {
				e.printStackTrace();
			}
			stageList.add(Str);
			...
			if (exist){
				if(!pass){
					if (CurrentStage!==stage.text){
						html+=
								'<span id = "stage-'+i+'" onclick=changeStage("'+stage.text+'") class="glyphicon glyphicon-ok-circle mystage" '
						html+= 'data-toggle="popover"'
						html+= 'data-placement="bottom"'
						html+= 'data-content="'+stage.text+'" style="color: #90F790;"></span>'
						html+= '-----------'
					}else {
						html+=
								'<span id = "stage-'+i+'" onclick=changeStage("'+stage.text+'") class="glyphicon glyphicon-map-marker mystage" '
						html+= 'data-toggle="popover"'
						html+= 'data-placement="bottom"'
						html+= 'data-content="'+stage.text+'" style="color: #90F790;"></span>'
						html+= '-----------'
					...
					if ("0"!==pMap[stage.text]){
						html+=
								'<span id = "stage-'+i+'" onclick=changeStage("'+stage.text+'") class="glyphicon glyphicon-record mystage" '
						html+= 'data-toggle="popover"'
						html+= 'data-placement="bottom"'
						html+= 'data-content="'+stage.text+'" style="color: #000000;"></span>'
						html+= '-----------'
					}else {
						html+=
								'<span id = "stage-'+i+'" onclick=changeStage("'+stage.text+'") class="glyphicon glyphicon-remove mystage" '
						html+= 'data-toggle="popover"'
						html+= 'data-placement="bottom"'
						html+= 'data-content="'+stage.text+'" style="color: #000000;"></span>'
						html+= '-----------'
				...
				if ("0"!==pMap[stage.text]){
					...
					html+= 'data-toggle="popover"'
					html+= 'data-placement="bottom"'
					html+= 'data-content="'+stage.text+'" ></span>'
					html+= '-----------'
				}else {
					if (CurrentStage!==stage.text){
						html+=
						...
						html+= 'data-toggle="popover"'
						html+= 'data-placement="bottom"'
						html+= 'data-content="'+stage.text+'" style="color: red;" ></span>'
						html+= '-----------'
					}
				}
			}
		...
		$("#stageDiv").html(html.slice(0,-11));
	}

	function changeStage(stage) {
		//发送ajax请求，实现stage的修改，同时添加一个交易历史
		$.ajax({
			...
			success:function (data) {
				if (data.success){
					//状态修改成功后，因为赋值是采用的request对象内的，而ajax没有打破该对象，无法修改，导致必须手动修改
					$("#edit-by").html(data.tranHistory.createBy);
					$("#edit-time").html(data.tranHistory.createTime);
					refreshStageIcon(stage)
				}else {
					alert("状态修改失败")
				}
			}
...
<body>
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 60px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${t.createBy}&nbsp;
				&nbsp;</b><small
		...
		<div style="position: relative; left: 40px; height: 30px; top: 70px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b id="edit-by">${t.editBy}&nbsp;
				&nbsp;</b><small
					style="font-size: 10px; color: gray;" id="edit-time">${t.editTime}</small></div>
			<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
