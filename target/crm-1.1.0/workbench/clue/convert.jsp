...
<script type="text/javascript">
	$(function(){
		...
		$("#createActModelBtn").click(function () {
			$("#queryinput").val("")
		...
		$("#setActSource").click(function () {
			let $selection = $("input[name=select_box]:checked");
			if ($selection==null){
				alert("请选择一个市场活动")
			}else {
				$("#activityId").val($selection.val())
				$("#activityName").val($("#n"+$selection.val()).html())
	...
	function QueryCandidateAct(){
		//填充 模态窗口，所有的活动必须和当前的clue有关联，关联关系内进行查找
		$.ajax({
			url:"workbench/clue/getActByClueId.do",
			type:"post",
			dataType:"json",
			data:{
				"clueId":"${param.clueId}",
			...
			success:function (data) {
				...
				$.each(data,function (i,n) {
					...
					html+='<tr>'
					html+='<td><input type="radio" name="select_box" value="'+n.id+'"/></td>'
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 90%;">
			<div class="modal-content">
						...
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" style="width: 300px;"
								   placeholder="请输入市场活动名称，支持模糊查询" id="queryinput">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="setActSource">提交</button>
				</div>
			</div>
		</div>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		<form id="create" method="post" action="workbench/clue/convert.do">
